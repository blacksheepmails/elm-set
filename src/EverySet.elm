...
-}
type Set t =
  Set_elm_builtin (EveryDict.EveryDict t ())
...
empty : Set a
empty =
  Set_elm_builtin EveryDict.empty
...
singleton : a -> Set a
singleton k =
  Set_elm_builtin <| EveryDict.singleton k ()
...
insert k (Set_elm_builtin d) =
  Set_elm_builtin <| EveryDict.insert k () d
...
remove k (Set_elm_builtin d) =
  Set_elm_builtin <| EveryDict.remove k d
...
union (Set_elm_builtin d1) (Set_elm_builtin d2) =
  Set_elm_builtin <| EveryDict.union d1 d2
...
intersect (Set_elm_builtin d1) (Set_elm_builtin d2) =
  Set_elm_builtin <| EveryDict.intersect d1 d2
...
diff (Set_elm_builtin d1) (Set_elm_builtin d2) =
  Set_elm_builtin <| EveryDict.diff d1 d2
...
filter p (Set_elm_builtin d) =
  Set_elm_builtin <| EveryDict.filter (\k _ -> p k) d
...
partition p (Set_elm_builtin d) =
  let
    (p1, p2) = EveryDict.partition (\k _ -> p k) d
  in
